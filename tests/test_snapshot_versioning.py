    ...
    def test_transaction_captures_initial_catalog_seq(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                # Initial catalog is at sequence 0
                assert catalog.seq == 0

                # Create transaction - should capture seq=0
                tblr = {0: 0, 1: 0}
                tblw = {0: 1}
                txn = Txn(1, env.now, 100, catalog.seq, tblr, tblw)
    ...
    def test_cas_fails_when_versions_differ(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                # Transaction at old sequence
                tblr = {0: 0}
                tblw = {0: 1}
                txn = Txn(1, env.now, 100, 0, tblr, tblw)  # v_catalog_seq = 0
                txn.v_dirty = {0: 0}
    ...
    def test_retry_updates_to_current_version(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                tblr = {0: 10, 1: 10}
                tblw = {0: 11}
                txn = Txn(1, 0, 100, catalog.seq, tblr, tblw)
